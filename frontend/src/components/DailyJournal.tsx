...
export function DailyJournal({ day, today, note, photoCount, onChanged }: { day: string; today: string; note: string | null; photoCount: number; onChanged: () => void }) {
  ...
  return <><div className="journal-actions"><button aria-label="Open daily note" title="Open daily note" className={note ? "journal-active" : ""} onClick={() => setMode("note")}><NotebookPen /></button><button aria-label="Open daily photos" title="Open daily photos" className={photoCount ? "journal-active" : ""} onClick={() => setMode("photos")}><ImagePlus /></button></div>{mode === "note" && <Modal title="Daily note" onClose={close}><form className="journal-editor" onSubmit={(event) => { event.preventDefault(); void saveNote(); }}><label>Note<textarea autoFocus rows={10} maxLength={20000} value={draft} disabled={!editable || saving} onChange={(event) => setDraft(event.target.value)} /></label><small>{draft.length.toLocaleString()} / 20,000</small>{error && <p className="error" role="alert">{error}</p>}<div className="draft-actions">{note && editable && <button type="button" className="danger" disabled={saving} onClick={() => void deleteNote()}><Trash2 />Delete</button>}<button type="button" onClick={close}>Cancel</button>{editable && <button className="primary" disabled={saving}>{saving ? "Saving…" : "Save note"}</button>}</div></form></Modal>}{mode === "photos" && <Modal title="Daily photos" onClose={close}><div className="photo-dialog">{editable && <><input ref={input} className="visually-hidden" type="file" accept="image/jpeg,image/png,image/heic,image/heif,image/webp" multiple onChange={(event) => void upload(event.target.files)} /><button className="primary" disabled={saving || photoCount >= 10} onClick={() => input.current?.click()}><ImagePlus />{saving ? "Uploading…" : "Add photos"}</button><p>{photos.length} of 10 photos</p></>}{error && <p className="error" role="alert">{error}</p>}<div className="photo-grid">{photos.map((photo) => <figure key={photo.id}><a href={photo.url} target="_blank" rel="noreferrer"><img src={photo.thumbnailUrl} alt={`Photo from ${day}`} /></a>{editable && <button className="icon-button danger" aria-label="Delete photo" onClick={() => void remove(photo)}><Trash2 /></button>}</figure>)}</div></div></Modal>}</>;
}
