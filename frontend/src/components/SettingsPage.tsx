...
export function SettingsPage({ config, pwaInstall }: { config: Config; pwaInstall: PwaInstallState }) {
  ...
  function changeSchedule(next: BackupSettings) {
    setSchedule(next);
  ...
  async function saveSchedule() {
    ...
    setScheduleError("");
    try {
      setSchedule(await api.updateBackupSettings(schedule));
      setScheduleSaved(true);
    } catch (caught) {
      setScheduleError(caught instanceof Error ? caught.message : "Could not save schedule.");
    } finally {
  ...
  return <main className="page settings-page">
    ...
    {schedule && <section className="settings-card"><h2>Backup schedule</h2><p>Server time: {config.timezone}. Daily keeps {schedule.dailyRetention}, and weekly keeps {schedule.weeklyRetention}. Each retention value may be set from 1 to 365.</p><label>Daily time<input type="time" value={schedule.dailyTime} onChange={(event) => changeSchedule({ ...schedule, dailyTime: event.target.value })} /></label><label>Daily retention<input type="number" min="1" max="365" value={schedule.dailyRetention} onChange={(event) => changeSchedule({ ...schedule, dailyRetention: Number(event.target.value) })} /></label><label>Weekly retention<input type="number" min="1" max="365" value={schedule.weeklyRetention} onChange={(event) => changeSchedule({ ...schedule, weeklyRetention: Number(event.target.value) })} /></label>{scheduleError && <p className="schedule-error" role="alert">{scheduleError}</p>}{scheduleSaved && <p className="schedule-success" role="status">Backup schedule saved.</p>}<button className={`secondary schedule-save-button${scheduleSaved ? " saved" : ""}`} disabled={savingSchedule} onClick={() => void saveSchedule()}>{scheduleSaved && !savingSchedule && <Check />}{savingSchedule ? "Saving…" : scheduleSaved ? "Saved" : "Save backup schedule"}</button></section>}
